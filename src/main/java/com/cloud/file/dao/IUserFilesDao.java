...
public interface IUserFilesDao {
    ...
    //根据用户id获取用户文件列表
    @Select("select * from t_files where username=#{username} AND isdelete=#{isdelete}")
    public List<Files> getAllUserFiles(@Param("username") String username, @Param("isdelete") Integer isdelete);

    //保存用户的文件记录
    @Insert("INSERT INTO t_files (oldFileName,newFileName,ext,path,size,type,isImg,downcounts,uploadTime,username,isdelete) VALUES (#{oldFileName},#{newFileName},#{ext},#{path},#{size},#{type},#{isImg},#{downcounts},#{uploadTime},#{username},#{isdelete})")
    ...
    //更新下载次数
    @Update("UPDATE t_files SET downcounts=#{downcounts} WHERE id=#{id}")
    ...
    //放入回收站
    @Update("UPDATE t_files SET isdelete=#{isdelete} WHERE id=#{id}")
    void updateDel(@Param("id") Integer id, @Param("isdelete") Integer isdelete);

    //search
    //AND username=#{username} AND isdelete=#{i}
    @Select("select * from t_files where oldFileName like #{text} AND username=#{username} AND isdelete=#{i}")
